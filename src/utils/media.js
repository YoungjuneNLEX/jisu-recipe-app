...
export async function fileToThumbnail(file) {
  if (!file) return null
  if (file.type.startsWith('image/')) return imageToThumbnail(file)
  if (file.type.startsWith('video/')) return videoToThumbnail(file)
...
function imageToThumbnail(file) {
...
function videoToThumbnail(file) {
  return new Promise((resolve, reject) => {
    ...
    video.onseeked = () => {
      try { resolve(drawToDataUrl(video, video.videoWidth, video.videoHeight)) }
      catch (e) { reject(e) }
      finally { cleanup() }
    }
